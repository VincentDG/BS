import numpy as np
import plotly.graph_objs as go
from flask import Flask, render_template, request
import wfdb

app = Flask(__name__)

# Define the available numbers
available_numbers = [
    "100", "101", "102", "103", "104", "105", "106", "107", "108", "109",
    "111", "112", "113", "114", "115", "116", "117", "118", "119",
    "121", "122", "123", "124",
    "200", "201", "202", "203",
    "205", "206", "207", "208", "209", "210",
    "212", "213", "214", "215",
    "217",
    "219", "220", "221", "222", "223",
    "228",
    "230", "231", "232", "233", "234"
]

# Define the available signals
available_signals = ["Signal 1", "Signal 2"]      # Placeholder

def plot_ecg(signal, end_time, fs=360.0):
    time = np.arange(0, len(signal)) / fs

    # Create a Plotly figure
    fig = go.Figure()

    # Add ECG signal trace
    fig.add_trace(go.Scatter(x=time, y=signal, mode='lines', name='ECG Signal'))

    # Customize layout
    fig.update_layout(
        title="ECG Plot",
        xaxis_title="Time (s)",
        yaxis_title="Amplitude (mV)",
        plot_bgcolor='white',
        height=800,  # Set the height to 800 pixels
        xaxis=dict(tickmode='array', tickvals=np.arange(0, end_time, 0.2), range=[0, end_time]),  
        yaxis=dict(range=[min(signal)-1, max(signal)+1]),  # Adjust the range of the y-axis
    )

    # Add light red gridlines every 0.04 seconds on the x-axis
    for x in np.arange(0, end_time, 0.04):
        fig.add_shape(type="line",
                      x0=x, y0=min(signal)-1, x1=x, y1=max(signal)+1,
                      line=dict(color="rgba(255, 0, 0, 0.5)", width=1, dash='solid'))  # Light red (alpha=0.5) solid line

    # Add darker red gridlines every 0.2 seconds on the x-axis
    for x in np.arange(0, end_time, 0.2):
        fig.add_shape(type="line",
                      x0=x, y0=min(signal)-1, x1=x, y1=max(signal)+1,
                      line=dict(color="rgba(255, 0, 0, 1.0)", width=1, dash='solid'))  # Darker red (alpha=1.0) solid line

    # Add light red gridlines every 0.1 on the y-axis
    for y in np.arange(0, max(signal)+1, 0.1):
        fig.add_shape(type="line",
                      x0=0, y0=y, x1=end_time, y1=y,
                      line=dict(color="rgba(255, 0, 0, 0.5)", width=1, dash='solid'))  # Light red (alpha=0.5) solid line

    for y in np.arange(0, min(signal)-1, -0.1):
        fig.add_shape(type="line",
                      x0=0, y0=y, x1=end_time, y1=y,
                      line=dict(color="rgba(255, 0, 0, 0.5)", width=1, dash='solid'))  # Light red (alpha=0.5) solid line

    # Add darker red gridlines every 0.5 on the y-axis
    for y in np.arange(0, max(signal)+1, 0.5):
        fig.add_shape(type="line",
                      x0=0, y0=y, x1=end_time, y1=y,
                      line=dict(color="rgba(255, 0, 0, 1.0)", width=1, dash='solid'))  # Darker red (alpha=1.0) solid line

    for y in np.arange(0, min(signal)-1, -0.5):
        fig.add_shape(type="line",
                      x0=0, y0=y, x1=end_time, y1=y,
                      line=dict(color="rgba(255, 0, 0, 1.0)", width=1, dash='solid'))  # Darker red (alpha=1.0) solid line

    return fig.to_html(full_html=False)

@app.route('/', methods=['GET', 'POST'])
def display_plot():
    if request.method == 'POST':
        # Get sampto value, selected number, and selected signal from the form
        sampto = int(request.form['sampto'])
        selected_number = request.form['selected_number']
        selected_signal = request.form['selected_signal']
    else:
        # Default sampto value
        sampto = 2000
        # Default selected number
        selected_number = "100"
        # Default selected signal
        selected_signal = "v0"

    # Read ECG signal data
    patient_record = wfdb.rdrecord(f"mitdb-1.0.0.physionet.org/{selected_number}", sampfrom=0, sampto=sampto)
    v0 = patient_record.p_signal[:, 0]
    v1 = patient_record.p_signal[:, 1]

    # Get the selected signal data
    signal_data = v0 if selected_signal == "v0" else v1

    # Plot ECG signal and return the HTML content
    plot_html = plot_ecg(signal_data, end_time=sampto / 360, fs=360.0)

    return render_template('plot.html', plot_html=plot_html, available_numbers=available_numbers, available_signals=available_signals, selected_number=selected_number, selected_signal=selected_signal)

if __name__ == '__main__':
    app.run(debug=True)